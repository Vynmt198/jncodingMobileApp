...
exports.enrollFreeCourse = async (req, res, next) => {
    try {
        ...
        if (Number(course.price) !== 0) {
            return res.status(400).json({ success: false, message: 'Chỉ khóa học miễn phí mới có thể đăng ký trực tiếp.' });
        }

        let enrollment = await Enrollment.findOne({ userId, courseId });
        if (enrollment) {
            if (enrollment.status === 'active' || enrollment.status === 'completed') {
                return res.status(200).json({ success: true, message: 'Bạn đã đăng ký khóa học này.', data: { enrollment } });
            }
            enrollment.status = 'active';
            await enrollment.save();
            return res.status(200).json({ success: true, message: 'Đã kích hoạt lại đăng ký.', data: { enrollment } });
        }

        enrollment = await Enrollment.create({ userId, courseId, status: 'active' });
        await Course.findByIdAndUpdate(courseId, { $inc: { enrollmentCount: 1 } });

        return res.status(201).json({ success: true, message: 'Đăng ký thành công.', data: { enrollment } });
    } catch (error) {
        next(error);
    }
};
